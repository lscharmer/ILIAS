...
class BuildChat
{
    ...
    {
        $room_tpl = new ilTemplate('tpl.chatroom.html', true, true, 'Modules/Chatroom');
        ...
        $room_tpl->setVariable('LBL_USER_TEXT', $this->ilLng->txt('invite_username'));

        $set_json_var('INITIAL_DATA', $initial);
        $set_json_var('INITIAL_USERS', $this->room->getConnectedUsers());

        $room_tpl->setVariable('CHAT_OUTPUT', $output); // $this->panel($this->ilLng->txt('messages'), $this->legacy('<div id="chat_messages"></div>'))
    ...
    private function renderLanguageVariables(ilTemplate $room_tpl): void
    {
        ...
        $vars = [
            ...
            'LBL_USER_IN_ILIAS' => 'user_in_ilias',
            'LBL_NO_USER' => 'msg_no_search_result',
